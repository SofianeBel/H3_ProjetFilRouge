...
export async function GET(request: NextRequest) {
  try {
    ...
    // SQLite: GROUP BY date(createdAt)
    const rows: { day: string; count: number; amount: number | null }[] = await prisma.$queryRawUnsafe(
      `SELECT date(createdAt) AS day, COUNT(*) AS count, SUM(amount) AS amount
       FROM orders
    ...
    for (const r of rows) {
      byDay.set(r.day, { count: Number(r.count || 0), amount: Number(r.amount || 0) })
    }
